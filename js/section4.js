$(function(){ 
	...
	var n=$('.section4-gallery-wrap>li').length;  //총이미지 갯수
		...
		$('.section4-gallery-wrap a').on({
			...
			//	$('#modal').show(0);
				$('.modalGallery').show(0);
				$('html').addClass('addModal'); //스크롤 없애기 
			}
		});
		
		
		function imgGalleryFn(z){ //imgGalleryFn(imgName) 이거에 번호가 전달되어 z로 변함
		...
		$('.modalGallery-next-Btn').on({
			click : function(){
				imgName++;
				if(imgName > 8){
					imgName=1; //8보다 큰 이미지는 없으니 1로 초기화 		
				}				//마지막 이미지이면 처음(1) 이미지로 설정 
			imgGalleryFn(imgName);	
			}
		});
		...
		$('.modalGallery-prev-Btn').on({
			click : function(){
				imgName--;
				if(imgName < 1){ 
					imgName=8; //1보다 작은 이미지는 없으니 마지막이미지 8로 설정 		
				}			
			imgGalleryFn(imgName);	
			}
		});
